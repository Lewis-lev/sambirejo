...
<body>
    ...
    <div class="grid">
            ...
            <table id="myTable" class="display">
                ...
                <?php $counter = 1; foreach ($pesanan as $row): ?>
                        <tr>
                            ...
                            <td class="bold" style="color: green;"><?= $row['date'] ;?></td>
                            <td style="text-align: center;" class="status">
                                <?php 
                                    if ($row['status']==1) {
                                        echo '<a href="table/status/'.$row['id'].'" class="btn btn-success">Sudah bayar</a>';
                                    } else {
                                        echo '<a href="table/status/'.$row['id'].'" class="btn btn-danger">Belum bayar</a>';
                                    }
                                    
                                ;?>
                                <?php 
                                    if ($row['kondisi']==1) {
                                        echo '<a href="table/kondisi/'.$row['id'].'" class="btn btn-success">Sudah berkunjung</a>';
                                    } else {
                                        echo '<a href="table/kondisi/'.$row['id'].'" class="btn btn-danger">Belum berkunjung</a>';
                                    }
                                    
                                ;?>
                            </td>
                            <td>
                                <a href="table/delete/<?= $row['id'] ;?>" class="btn btn-danger"><i class="fa fa-trash" style="font-size:20px;"></i></a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
        ...
        <a href="table/deleteAll" class="btn btn-danger center">Delete All</a>
